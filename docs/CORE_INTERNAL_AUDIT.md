...
C2.1 — Santé Core Swift : terminé.
C2.2 — Clarification textes Lab UI : terminé.
C2.3 — Tests de garde routes / state / UI : terminé.
C2.4 — Observation terrain post-hardening : terminé.
C2.5 — Contrat `/feed` + observation app Swift : terminé.
...
Note C2.5 : ce contrat est maintenant verrouillé par test. `/feed` ignore explicitement des événements bruts non promus comme `app_activated`, `file_modified`, `user_presence`, `screen_locked` et `screen_unlocked`, et ne doit pas devenir un équivalent de `/insights`.
...
- fait en C2.3 : inventaire ciblé des routes Core / debug / Lab enregistrées ;
- fait en C2.3 : garde anti-ajout de champs sensibles dans `/state.present` ;
- test que `/insights` est explicitement debug/raw dans un metadata ou doc contract ;
- fait en C2.5 : contrat `/feed` comme sélection notable, pas journal brut complet ;
