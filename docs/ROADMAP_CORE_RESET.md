...
- [ ] `docs/ROADMAP_CORE_RESET.md` existe.
- [ ] Le document est relu et compris comme document maître.
- [ ] Le document est commit sur la branche de recadrage.
- [ ] Le README indique clairement que Pulse est en Core Reset.
- [ ] Le README distingue Core stable et Lab expérimental.
- [ ] Le README indique que les anciennes roadmaps sont secondaires pendant le Core Reset.
- [ ] Les anciennes roadmaps principales reçoivent un avertissement ou un renvoi vers ce document si nécessaire.
- [ ] Les anciennes docs ne sont pas supprimées, mais deviennent secondaires si elles contredisent ce document.
- [ ] Aucune tâche DayDream, facts, LLM, adaptation ou dashboard avancé n’est prioritaire pendant R0/R1.
- [ ] La prochaine tâche technique officielle est l’introduction du mode runtime.
- [ ] `git status` ne montre plus `docs/ROADMAP_CORE_RESET.md` comme fichier non suivi.
...
- [ ] Ajouter une configuration `PULSE_MODE`.
- [ ] Créer un module central dédié au mode runtime, par exemple `daemon/runtime_mode.py`, plutôt que placer cette logique dans une policy LLM.
- [ ] Ajouter une fonction centralisée `get_pulse_mode()`.
- [ ] Ajouter une fonction centralisée `is_lab_enabled()` pour activer Lab uniquement en mode `lab` ou `dev`.
- [ ] Définir `core` comme valeur par défaut.
- [ ] Exposer `pulse_mode` dans `/state`.
- [ ] Exposer `experimental_enabled: false` en mode Core.
- [ ] Exposer les détails plus verbeux uniquement dans `/debug/state` si nécessaire.
- [ ] Ajouter des tests unitaires de parsing du mode runtime.
- [ ] Ajouter des tests sur les payloads `/state` ou builders d’état.
