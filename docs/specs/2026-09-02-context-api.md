...
> courante. Les contraintes de fichiers inchangés et plans d’implémentation
> ci-dessous décrivent uniquement la livraison initiale. Le contrat JSON de
> contexte reste v2. Voir la [décision actuelle](../decisions/2026-09-08-reconstruction-unique-des-sessions.md).
...
| `current_session` | La session de travail (jamais une activité isolée) dont `last_activity_at` est la plus récente **et** postérieure à `reference_at − session_gap` (30 min ; seuil désormais possédé uniquement par `analysis.timeline`). `is_open` = `true` dans ce cas. Si aucune session ne satisfait ça, `current_session` vaut `null`. On ne met **pas** la dernière session fermée à la place : « rien en cours » est une information. Les sessions sont reconstruites jour local par jour local, comme la trace quotidienne, sur la journée entière jusqu'à `reference_at` : une session commencée avant la fenêtre garde ses vraies bornes. Identifiant : le hash stable (voir la ligne suivante) ; `label` porte l'ordinal `work-N`. **Fermeture monotone** (décision du 2026-09-03, `reconstruction_version` 2) : un `screen_locked` ou un `system_sleep` ferme la session sur-le-champ, sur son dernier travail observé, avec ce motif, et rien ne la rouvre — `is_open: false` ne se défait jamais avec plus de données, et `current_session` ne renvoie **jamais** une session explicitement fermée : dans la minute qui suit un verrouillage ou une veille, encore dans le gap, elle est déjà `null` ici et `is_open: false` dans `/context/sessions` — les deux vues lisent le même `end_reason` (correctif du 2026-09-06, défaut 1 de l'audit). L'activité forte observée avant la reprise correspondante (`screen_unlocked` / `system_wake`) est une vue `activity_kind: background`, jamais `current_session`, `recent_sessions` ni une entrée de `/context/sessions`. |
...
- Verrouillage ou veille à l'instant, dernier travail encore dans le gap → `current_session: null`, la session est déjà `is_open: false` dans `/context/sessions`.
...
- **Minuit local — divergence résolue le 2026-09-08.** La trace quotidienne ferme toute session à minuit (`day_boundary`). `/context` respecte désormais cette fermeture et expose la session dans l’historique récent, jamais comme session courante.
