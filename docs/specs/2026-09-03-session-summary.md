...
Chaque passage commence par rejouer les payloads `pending`, **avant** la sélection et indépendamment de la fenêtre `lookback_days` : un résumé gelé pendant une panne Core repart au premier passage après rétablissement, même plusieurs jours plus tard, sans commande datée. Le rejeu est octet pour octet, `entry["event"]` tel que figé : ni `generated_at`, ni `generation_ms`, ni `input_hash`, ni `producer` ne sont recalculés, et les versions enregistrées avec l'entrée servent à l'inscription dans `emitted`. Le budget d'échecs s'applique comme à toute émission ; un rejeu refusé compte `failed` dans le bilan (`replayed=` dans la ligne de `run`, codes de sortie inchangés). Un `409` au rejeu n'est pas un refus ordinaire : il passe par la récupération après conflit (ci-dessous), pas par le budget. Un `pending` d'une session `given_up` n'est pas rejoué par le vidage : il reste sur disque jusqu'à une reprise explicite (`summarize <id> --retry`). Le saut post-vidage porte sur l'**identité** rejouée dans le passage (`event_id`), jamais sur la session : une identité rejouée, acceptée ou refusée, n'est pas re-POSTée par la sélection du même passage, et une session dont le `pending` est abandonné sous un ancien prompt ou modèle redevient candidate sous une identité neuve (relecture du 2026-09-06, sonde A). Ce vidage est distinct du rattrapage des sessions jamais traitées, qui relève d'une politique de curseur séparée, non livrée.
...
Récupération après conflit (relecture du 2026-09-06, sonde B) : si le rejeu d'un `pending` reçoit `409`, Core détient déjà un événement sous cette identité avec un autre contenu — un état perdu puis régénéré entre une sauvegarde et sa restauration. Intelligence lit alors `GET /activities/<event_id>` : `200`, l'entrée est enregistrée telle que Core l'a stockée (`origin: "core"`), le `pending` est retiré, statut `already_known` (« récupéré depuis Core après conflit »), budget intact ; `404`, cas théorique où Core refuse sans rien détenir, refus ordinaire compté comme une tentative. Le `409` de Core reste intact : c'est Intelligence qui se range à ce que Core a accepté.
