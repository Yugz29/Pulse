...
class TestDebugMemoryViews(unittest.TestCase):
    ...
    def test_commit_episode_links_utilisent_la_fenetre_fichier_du_journal(self):
        ...
        self.assertEqual(link["episode_started_at"], matching_episode["started_at"])
        self.assertEqual(link["episode_ended_at"], matching_episode["ended_at"])
        self.assertEqual(link["evidence_candidate_id"], "journal-file-window-journal-1")
        self.assertEqual(link["evidence_episode_id"], "journal-file-window-journal-1")
        self.assertEqual(link["evidence_started_at"], observed_at.replace(hour=14, minute=26).isoformat())
        self.assertEqual(link["evidence_ended_at"], observed_at.replace(hour=14, minute=29).isoformat())
