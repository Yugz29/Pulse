...
class TestRuntimeOrchestrator(unittest.TestCase):
    ...
    def test_sync_memory_background_enqueue_lightweight_commit_summary_si_queue_presente(self):
        ...
        self.assertIn("changed_scope", pending.prompt)
        self.assertIn("diff --git a/runtime.py b/runtime.py", pending.prompt)
        self.assertNotIn("Diff compact :", pending.prompt)
        self.assertNotIn("Faits observés :", pending.prompt)
        self.assertNotIn("Changements détectés", pending.prompt)
        self.assertNotIn("ajoute une route GET /llm/lightweight/status", pending.prompt)
